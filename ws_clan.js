function clan() {
    ...
    var siglas = document.querySelector("#datosclan > tbody > tr:nth-child(1) > td:nth-child(3)").innerText.split("(")[1].replace(")", "");
    ...
    $(".lista1 tr").each(function(index, obj) {
        ...
        if (Math.abs(diferenciaMinima) <= tolerancia) {
            if (diferenciaMinima > 0) subMensaje = "<br>a " + Math.abs(diferenciaMinima) + " puntos de salir";
            else subMensaje = "<br>-" + Math.abs(diferenciaMinima) + " puntos para entrar";
        }
        if (Math.abs(diferenciaMaxima) <= tolerancia) {
            if (diferenciaMaxima > 0) subMensaje = "<br>a " + Math.abs(diferenciaMaxima) + " puntos de salir";
            else subMensaje = "<br>+" + Math.abs(diferenciaMaxima) + " puntos para entrar";
        }
        if (enRango) mensaje = "<br><span style='color: #006400'>[En rango]</span>" + subMensaje;
        else mensaje = "<br><span style='color: #990000'>[Fuera de rango]</span>" + subMensaje;
...
function cargarClan() {
    ...
    if (document.getElementById("_ayudam1") == null) return miClan;
    miClan.maravilla1 = document.querySelector("#_ayudam1 h3").innerText;
    if (document.getElementById("_ayudam2") == null) return miClan;
